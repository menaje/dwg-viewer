...
VS Code 확장에서는 파일을 매번 고르는 대신 도면 폴더와
`dwgViewer.shxFontDirectories`에 등록한 폴더를 첫 화면 뒤에만
검색합니다. 도면이 실제로 요구한 SHX/BigFont 이름만 읽으며, 이름이
다른 대체 파일은 `dwgViewer.shxFontMappings`로 연결합니다. 검색은
하위 폴더로 확장하지 않고 요청을 직렬 처리하며, 파일당 32MiB,
도면당 전송 64MiB, 128개 요청으로 제한합니다. 글꼴 패널은 연결,
대체, 누락, 손상, 읽기 실패와 한도 초과를 구분해 표시합니다.
...
처음 여는 대형 도면은 Native 변환이 끝날 때까지 빈 화면으로 기다리지
않습니다. 같은 변환 프로세스가 해석 직후 4MiB 제한 개요가 든 독립
미리보기 캐시를 먼저 내보내며, Webview는 이를 표시한 채 전체 상세
캐시 준비를 계속합니다. 전체 캐시의 첫 화면이 그려지면 미리보기 파일과
읽기 채널을 정리합니다. 미리보기 실패나 이를 모르는 구형 어댑터는 기존
전체 캐시 경로로 안전하게 계속됩니다.
...
새 빠른 미리보기 경로의 분리 측정에서는 같은 기준 도면의
4,914,376바이트 첫 화면 캐시가 변환 시작 1,241ms에 준비됐고, 전체
177,049,408바이트 캐시는 3,312ms에 끝났습니다. 실제 Chromium 첫
화면은 각각 517.9ms와 502.6ms였으므로 첫 선 화면을 약 2.1초 앞당깁니다.
미리보기 JavaScript 힙 최고값은 29.41MiB로 전체 캐시의 60.16MiB보다
낮았고, 최종 캐시 SHA-256은 기존 결과와 동일했습니다.
