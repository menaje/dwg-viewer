...
범위 읽기·생성하고, POINT·SOLID·3DFACE 역시 상한이 있는 일회성
워커에서 처리하므로 초기 로딩을 막지 않습니다.
...
뷰에서 보이는 가장자리만 지연 표시합니다. 남은 엔티티 계열 및 정밀
...
POINT·SOLID·3DFACE 원본은 첫 화면 범위에 포함하지 않으며, 별도 워커가
공유 블록 인스턴스를 유지한 채 최대 32MiB의 GPU 버퍼로 변환합니다.
...
POINT·SOLID·3DFACE는 첫 선 화면 뒤 HATCH보다 먼저 일회성 워커에서
읽습니다. POINT 표시는 8MiB, SOLID 채움은 16MiB, SOLID와 3DFACE가
공유하는 외곽선은 8MiB로 각각 제한하고, 결과를 전송하면 워커를
종료합니다. 따라서 원본 버퍼를 화면 프로세스에 계속 보관하지 않고
HATCH 원본 처리와의 순간 메모리 중첩도 피합니다.
